...
// Define TypeScript interfaces for component props and data
      ...
      <div className="w-full">
        <div className="flex h-16 items-center px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center">
            ...
            <div className="flex items-center">
              <Package2 className="h-8 w-8 text-indigo-400" />
              ...
              {isSearchOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-slate-800 rounded-lg shadow-2xl z-20 overflow-hidden border border-slate-700">
                  <div className="p-3">
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-700 text-xs text-slate-500 flex justify-between">
                      <span>Press <kbd className="px-1 py-0.5 bg-slate-700 rounded">↩</kbd> to select</span>
              ...
              {isProfileDropdownOpen && (
                <div className="absolute right-0 mt-3 w-56 bg-slate-800 border border-slate-700 rounded-lg shadow-2xl z-20 overflow-hidden">
                  <div className="py-3 px-4 border-b border-slate-700">
                    ...
                    <div className="text-xs text-slate-400">john.smith@company.com</div>
                    <div className="text-xs text-indigo-400 mt-1">Administrator</div>
                  </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-slate-800 border-b border-slate-700">
          <div className="py-2 px-4">
            <MobileMenuItem icon={<Boxes />} label="Inventory" alert={lowStockCount > 0} alertCount={lowStockCount} />
            <MobileMenuItem icon={<ShoppingCart />} label="Orders" alert={pendingOrdersCount > 0} alertCount={pendingOrdersCount} />
            <MobileMenuItem icon={<Truck />} label="Suppliers" />
            <MobileMenuItem icon={<BarChart3 />} label="Analytics" />
...
// Quick Link Button Component (desktop view)
...
const QuickLinkButton: React.FC<QuickLinkButtonProps> = ({ 
  ...
  <button className="relative flex items-center px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-md transition-colors">
    <span className="mr-2">{icon}</span>
    <span>{label}</span>
    {alert && (
      <span className="absolute -top-1 -right-1 bg-indigo-600 text-white rounded-full h-5 w-5 flex items-center justify-center text-xs font-bold">
        {alertCount}
      </span>
    )}
  </button>
...
const MobileMenuItem: React.FC<MobileMenuItemProps> = ({ 
  ...
  <button className="relative w-full flex items-center py-3 text-sm text-slate-300 hover:text-white hover:bg-slate-700 rounded-md transition-colors px-2">
